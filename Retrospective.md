...
- Result of the DOM/HAR isolation experiment: all six medians passed, but 88/80/176/176/200/168ms did not consistently improve the 64/40/192/160/192/168ms baseline; a 224ms individual event remained. The configuration change was rejected and the original full trace configuration restored. Passing one run is insufficient evidence of a root-cause correction.
- Further evidence is limited to native browser version and allowlisted GPU renderer/device/feature fields read before measured contexts. Local Chromium also reports software rendering, so a hosted-only GPU defect must not be assumed. Keep the approved visuals, runner, browser channel and budgets unchanged until a safe correction is supported by actual evidence.
