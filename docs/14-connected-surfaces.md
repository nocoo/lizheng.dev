...
Profiling found that cached, invisible device trees still participated in repeated style work during theme changes. Cached layers now use `content-visibility: hidden`, retaining their prepared DOM and state while deferring rendering until they enter. Entering, current and exiting layers keep their full rendering and overflow, preserving the approved motion, props and shadows. A local 12× CPU diagnostic trace reduced the longest style update from 49.6ms to 21.4ms; this diagnostic is separate from the unchanged 4× CPU release gate.
...
The user authorized another patch if needed. The [v3.1.2 release record](https://github.com/nocoo/lizheng.dev/releases/tag/v3.1.2) records final main CI, validated-artifact deployment and the mandatory five-minute follow-up as they complete. No performance threshold, retry setting, visual baseline or animation duration was changed.
