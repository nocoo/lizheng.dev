...
The SEO extension in [15](15-seo-agent-social.md) adds two development regressions, bringing this suite to ten checks. The new tests first reproduced `/llms.txt` returning 404 on both surfaces after its production generation moved into the build. Preview now generates the same bilingual document from the public model, emits the HTML/Markdown discovery headers, and returns a true 404 for unknown personal-page URLs. All ten development checks passed for that implementation. Preview uses only `X-Robots-Tag: noindex`: crawlers may read the preview and follow its links, while indexing the canonical production pages. Production responses do not set this header. Root language redirects and existing legacy 301s retain their order.
