...
本轮只完成 M0 文档并汇报。以下阶段是后续重构的执行顺序，不代表已经实现或部署。

## 提交原则

在 main 上按一个可解释行为/模块一次提交。先写失败测试，在工作区完成 Red/Green/Refactor，然后把测试与实现作为一个绿色提交；不提交失败 main、不跳过 Husky、不用大规模“最终整理”替代持续零 lint。
