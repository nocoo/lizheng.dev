...
M0 与两站设计迭代已完成，用户已批准设计并授权完整门禁、main 推送和 Cloudflare 部署。v3.0.0 已先行上线；剩余质量工作继续完成。实际提交与验证见 11。以下表格是阶段范围，当前状态不再由早期的设计优先限制决定。
...
现行 CI 先完成质量检查和浏览器矩阵，再上传按 commit SHA 命名的 Worker/资产包。Release 验证 CI 来源、分支、结果和仓库，仅部署该产物；手动执行也必须指定已通过的 main CI run ID。
