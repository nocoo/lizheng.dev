...
Prepared scenes retain their DOM in a cache bounded to six layers. Chromium and WebKit first give hidden layers layout with `visibility: hidden`. After the final mount has crossed a render frame, `content-visibility: hidden` suspends their rendering until activation. Firefox retains its `display: none` cache because of the skipped-subtree style cost described below. All cached layers remain `inert` and `aria-hidden`, accept no pointer input and run no CSS animations or transitions. Inactive devices skip React updates until activation, when they receive the current shared selection and profile state; decorative SVGs are memoized. This avoids constructing a complete machine during the click while preserving the original entrances, geometry and native controls.
...
## Cached layout follow-up, 2026-09-11

The v3.2.0 [release CI](https://github.com/nocoo/lizheng.dev/actions/runs/34556255675) passed the browser and development checks but measured 224 ms for English mobile Play interactions. A controlled rerun of the unchanged source still exceeded the 200 ms limit: 280 ms on mobile and 208 ms on desktop. Both attempts are retained. The same mobile scenario measured 80 ms locally under the release test's 4× CPU and network conditions, so the local pass alone was insufficient for publication.

A separate local Chromium CDP trace at 12× CPU identified full device layout on chapter changes. `display: none` preserved cached DOM but discarded its layout, moving that work back into the click. Cached layers now retain their geometry with `visibility: hidden`; the existing idle mounting spreads preparation across idle callbacks. Their absolute positioning, `inert`, `aria-hidden`, disabled pointer input and stopped animations remain in place. The renderer, six scene identities, artwork and transition keyframes are unchanged.

Under the same 12× diagnostic conditions, the longest layout during the interaction sequence fell from 176.1 ms to 19.5 ms, and the maximum measured click fell from 272 ms to 168 ms. These are individual diagnostic traces, separate from the mandatory six-scenario, three-sample release gate at 4× CPU. Traces, per-event timings and both CI failures are retained in `.design-review/release-3.2.0/`; the [v3.2.0 release record](https://github.com/nocoo/lizheng.dev/releases/tag/v3.2.0) records the final CI and production outcome.

After the change, all six local cold-load scenarios passed with the standard throttled network, 4× CPU and three samples per scenario. Play interaction medians were 48–80 ms and résumé medians were 32–40 ms; LCP and CLS also passed their existing limits. All 85 Chromium browser checks and 26 focused Firefox/WebKit checks covering prepared scenes and Play passed without skipped, unexpected or flaky results. Existing screenshot baselines passed unchanged. Remote CI remains mandatory before deployment.

The [next CI](https://github.com/nocoo/lizheng.dev/actions/runs/34558425258) completed all 255 browser and eight development checks after one controlled Chromium rerun. Its first attempt had one Nokia transition cleanup timeout; the trace was retained and six unchanged local repetitions passed. The rerun passed that browser case but still blocked deployment on the English desktop interaction median of 232 ms. All three desktop samples identified theme switching as the slowest interaction; the other five performance scenarios passed.

The follow-up separates layout preparation from rendering suspension. Two animation-frame callbacks after the final idle mount allow the hidden scenes to receive layout before a gallery marker enables `content-visibility: hidden` on cached layers. This differs from the earlier cache that skipped rendering immediately on mount. The visible scene still uses its original layout, palette and transitions. The page retains its background-color fade and switches inherited text color directly, avoiding repeated style calculation through the entire cache.

A separate 12× CPU desktop CDP comparison reduced the main theme style pass from 1,130 elements / 49.1 ms to 336 elements / 19.2 ms. Its theme click changed from 152 ms to 120 ms and returning to Game Boy from 144 ms to 104 ms. These diagnostic samples do not replace the standard release gate. Palette, texture and external-SVG experiments that did not improve the measured interaction were not adopted; their traces remain in `.design-review/release-3.2.0/`.

The first complete local matrix with rendering suspension passed all 255 checks without screenshot changes, but Firefox's shared-frame scenario rose from 2.5 s to 19.3 s under the same single-worker conditions. A Firefox-specific `-moz-appearance` support query retains the previous `display: none` cache in that engine; Chromium and WebKit keep the prepared-layout suspension. This preserves the existing Firefox behavior instead of accepting the measured style-query regression. No assertions, screenshots or timeouts are relaxed.

The Firefox follow-up passed all 85 cases and returned the shared-frame scenario to 2.3 s, even with the full suite using three workers. Direct checks of the support query confirmed that it selects Firefox only; Chromium and WebKit retain the rendering behavior already verified by their 85-case runs. All existing visual baselines remain unchanged.

The final standard six-scenario performance run passed with the unchanged 4× CPU, network throttling and three samples per scenario. Play interaction medians were 40/64 ms in English and 32/64 ms in Chinese at 390/1440px; résumé medians were 32/40 ms. LCP and CLS remained within their existing limits. All ten development/HMR checks also passed. The final source still requires a successful remote CI and validated-artifact deployment before publication.
