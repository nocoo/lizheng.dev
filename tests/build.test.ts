...
import { getValue, findMatchingClose, processEach, processIf, processVariables } from "../build.js";
...
describe("findMatchingClose", () => {
  ...
  test("no closing tag returns -1", () => {
    expect(findMatchingClose("{{#each a}}hello", "{{#each ", "{{/each}}", 11)).toBe(-1);
  });
});
...
describe("processEach", () => {
  ...
  test("array of objects", () => {
    ...
    const result = processEach(template, { people: [{ name: "Alice" }, { name: "Bob" }] });
...
describe("processVariables", () => {
  test("simple variable", () => {
    expect(processVariables("Hello {{name}}", { name: "World" })).toBe("Hello World");
  });
  ...
  test("triple braces (unescaped)", () => {
    expect(processVariables("{{{html}}}", { html: "<b>bold</b>" })).toBe("<b>bold</b>");
  });
  ...
  test("multiple variables", () => {
    expect(processVariables("{{a}} and {{b}}", { a: "1", b: "2" })).toBe("1 and 2");
  });
});
